...
{
public:
    ExtrusionEntityCollection *clone() const override;
    ...
    ExtrusionEntityCollection(const ExtrusionEntityCollection &other)
        : orig_indices(other.orig_indices), no_sort(other.no_sort)
    {
        this->append(other.entities);
    }
    ExtrusionEntityCollection(ExtrusionEntityCollection &&other)
        : entities(std::move(other.entities))
        , orig_indices(std::move(other.orig_indices))
    ...
    ExtrusionEntityCollection &operator=(ExtrusionEntityCollection &&other)
    {
        ...
        this->orig_indices = std::move(other.orig_indices);
    ...
    void append(const ExtrusionEntitiesPtr &entities)
    {
        ...
        for (ExtrusionEntitiesPtr::const_iterator ptr = entities.begin();
             ptr != entities.end(); ++ptr)
    ...
    ExtrusionEntityCollection chained_path(bool          no_reverse = false,
                                           ExtrusionRole role = erMixed) const;
    void chained_path(ExtrusionEntityCollection *retval,
                      bool                       no_reverse = false,
                      ExtrusionRole              role       = erMixed,
                      std::vector<size_t> *orig_indices     = nullptr) const;

    ExtrusionEntityCollection chained_path_from(
        Point         start_near,
        bool          no_reverse = false,
        ExtrusionRole role       = erMixed) const;

    void chained_path_from(Point                      start_near,
    ...
    // Produce a list of 2D polygons covered by the extruded paths, offsetted
    // by the extrusion width. Increase the offset by scaled_epsilon to
    // achieve an overlap, so a union will produce no gaps.
    void polygons_covered_by_width(Polygons &  out,
                                   const float scaled_epsilon) const override;

    // Produce a list of 2D polygons covered by the extruded paths, offsetted
    // by the extrusion spacing. Increase the offset by scaled_epsilon to
    // achieve an overlap, so a union will produce no gaps. Useful to
    // calculate area of an infill, which has been really filled in by a 100%
    // rectilinear infill.
    void polygons_covered_by_spacing(
        Polygons &out, const float scaled_epsilon) const override;
    ...
    {
        double volume = 0.;
        for (const auto &ent : entities) volume += ent->total_volume();
        return volume;
    }
