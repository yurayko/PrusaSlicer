...
bool GLGizmosManager::init()
{
    ...
    m_gizmos.emplace_back(new GLGizmoFdmSupports(m_parent, "sla_supports.svg", 6));
...
{
    if (!m_enabled || (m_current != SlaSupports && m_current != FdmSupports) || m_gizmos.empty())
...
bool GLGizmosManager::on_mouse_wheel(wxMouseEvent& evt)
{
    ...
    if (m_current == SlaSupports || m_current == FdmSupports) {
...
bool GLGizmosManager::on_mouse(wxMouseEvent& evt)
{
    ...
    if (get_gizmo_idx_from_mouse(mouse_pos) == Undefined)
    {
        ...
        else if (evt.RightDown() && (selected_object_idx != -1) && (m_current == SlaSupports) && gizmo_event(SLAGizmoEventType::RightDown))
        {
            ...
            processed = true;
        else if (evt.Dragging() && (m_current == SlaSupports || m_current == FdmSupports )
        ...
        else if (evt.LeftUp() && (m_current == SlaSupports || m_current == FdmSupports) && !m_parent.is_mouse_dragging())
        {
...
bool GLGizmosManager::on_char(wxKeyEvent& evt)
{
    ...
    else if (!evt.HasModifiers())
    {
        ...
        case 'R' :
        {
            if ((m_current == SlaSupports || m_current == FdmSupports) && gizmo_event(SLAGizmoEventType::ResetClippingPlane))
                processed = true;
